...
export default function AdminPage() {
  ...
  async function handleAddOrUpdateChapter(courseId, e) {
    ...
    let pdfDocumentUrl = newChapter.pdfDocument;
    
    // Handle PDF file upload if a file is selected
    if (pdfFile) {
      try {
        ...
        if (response.ok) {
          ...
          pdfDocumentUrl = result.downloadURL;
        } else {
                    ...
                    <div className="flex justify-between">
                          ...
                          <p className="text-sm text-gray-600">{ch.topics}</p>
                           <div className="flex flex-wrap gap-2 mt-2">
                            ...
                            {ch.pdfDocument && <a href={ch.pdfDocument} target="_blank" rel="noopener noreferrer" className="text-red-500 underline text-sm">PDF Document</a>}
                            {ch.classDocs && <span className="text-orange-500 text-sm">📄 {ch.classDocs}</span>}
                          </div>
                        </div>
                                             <div className="flex gap-2">
                         <button 
                    ...
                    {editingChapterId === ch.id && (
                      <div className="mt-4 border-t pt-4 bg-gray-50 p-3 rounded">
                        ...
                        <form onSubmit={(e) => handleAddOrUpdateChapter(course.id, e)} className="grid grid-cols-2 gap-2">
                          ...
                          <input className="border p-2 rounded" placeholder="Accent Docs (Optional)" value={newChapter.classDocs || ""} onChange={(e) => setNewChapter({ ...newChapter, classDocs: e.target.value })} />
                           ...
                           {pdfFile && (
                             <div className="mt-2 p-2 bg-green-50 border border-green-200 rounded">
                               <div className="flex items-center gap-2">
                                 ...
                                 <span className="text-sm text-green-800 font-medium">Valid PDF file selected</span>
                               </div>
                               <p className="text-xs text-green-600 mt-1">
                                 File: {pdfFile.name} ({(pdfFile.size / 1024 / 1024).toFixed(2)} MB)
                               </p>
                             </div>
                           )}
                         </div>
